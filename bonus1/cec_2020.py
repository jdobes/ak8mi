...
def main():
    ...
    algos = {"simulated_annealing": simulated_annealing}

    # Evaluate test functions using simulated_annealing
    ...
    for func_id in FUNCTIONS:
        for dimension in FUNCTIONS_TO_DIMENSIONS[func_id]:
            for algo_name, algo_lib in algos.items():
                ...
                best_run_results = []
                for i in range(RUNS):
                    best_results = algo_lib(BOUNDS, dimension, cost_function, func_id, DIMENSION_TO_MAX_FES[dimension])
    ...
    for func_id in FUNCTIONS:
        for dimension in FUNCTIONS_TO_DIMENSIONS[func_id]:
            ...
            plt.ylabel("cost")
            plt.grid(True)
            #plt.axis((0, DIMENSION_TO_MAX_FES[dimension], 0, 100000))  # use to zoom
